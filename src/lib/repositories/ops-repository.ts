...
function toOrder(row: Record<string, unknown>): DispatchOrder {
  return {
    ...
    driverAckStatus: optionalText(row, "driver_ack_status") as DispatchOrder["driverAckStatus"],
    driverAckCount: optionalNumber(row, "driver_ack_count"),
    driverAckLastSentAt: optionalText(row, "driver_ack_last_sent_at"),
    driverAcknowledgedAt: optionalText(row, "driver_acknowledged_at"),
    driverAckEscalatedAt: optionalText(row, "driver_ack_escalated_at"),
...
function fromOrder(order: DispatchOrder) {
  return {
    ...
    driver_ack_status: order.driverAckStatus ?? null,
    driver_ack_count: order.driverAckCount ?? null,
    driver_ack_last_sent_at: order.driverAckLastSentAt ?? null,
    driver_acknowledged_at: order.driverAcknowledgedAt ?? null,
    driver_ack_escalated_at: order.driverAckEscalatedAt ?? null,
