...
import {
  appOrderActionUrl,
  buildDispatchProposalIntegrationPayload,
  buildDriverReportNote,
  buildTransportCode,
  calendarEventClass,
  canViewTab,
  contractTypeLabels,
  customerRecognitionFullLabel,
  customerRecognitionOptions,
  customerConfirmationText,
  customerSourceCodeOptions,
  customerSourceFullLabel,
  dateKey,
  dateOnly,
  defaultOrderManagerName,
  defaultOrderTimes,
  dispatchLabels,
  driverActionDetail,
  driverActionLabel,
  driverNextDispatchStatus,
  driverOptionLabel,
  driverReportNoteParts,
  durationMinutes,
  escapeHtml,
  finalDispatchOrderText,
  formatDateTime,
  getMonthCells,
  guestMarketCode,
  guestMarketLabel,
  ...
  ownerCompanyProfile,
  parseRouteLegs,
  paymentCollectorInfo,
  paymentLabels,
  paymentMethodLabels,
  primaryLegValues,
  priorityLabels,
  provinceCodeOptions,
  provinceRouteFullLabel,
  quoteLabels,
  quoteTone,
  resolveOrderTransport,
  resourceStatusLabels,
  roleHomeTab,
  routeLegsForOrder,
  routeSummaryForOrder,
  safeOrderFileCode,
  salesOwnerOptions,
  serviceOptions,
  statusTone,
  ...
  vietnamDateTimeLiveLabel,
  vietnamDateTimeLocalValue,
  vietnamFriendlyDate,
  vietnamMonthLabel,
  vietnamTimeZone,
  type ServiceCode,
  type Tab
} from "@/lib/ops-display";
import {
  assignVehicleDriver,
...
import { Badge, Field, RoleAccountControls, SectionDetails, StatCard, greetingName, readableAuthName } from "@/components/ops-ui";
import type {
  Assignment,
  AuditEvent,
  AppNotification,
  ...
  async function assignOrder(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (assignmentMode === "rented") {
        ...
        window.dispatchEvent(new CustomEvent("dispatch-assignment-saved", { detail: { orderId: selectedOrder.id } }));
...
function DispatchPanel({
  ...
  const [mobileView, setMobileView] = useState<"overview" | "orders" | "detail" | "schedule" | "vehicleDetail" | "assign" | "supplier">("overview");
  const [desktopView, setDesktopView] = useState<"desk" | "orders" | "detail" | "schedule" | "assign" | "supplier">("desk");
  ...
  const [showDispatchAlerts, setShowDispatchAlerts] = useState(false);
  useEffect(() => {
    const onSaved = (event: Event) => {
      if ((event as CustomEvent<{ orderId: string }>).detail.orderId === selectedOrder.id) setAssignmentStep(2);
    };
    window.addEventListener("dispatch-assignment-saved", onSaved);
    return () => window.removeEventListener("dispatch-assignment-saved", onSaved);
  }, [selectedOrder.id]);
  ...
  const urgentOrders = dispatchBaseQueue
  ...
  function selectOrder(orderId: string, nextView: typeof mobileView = "detail") {
    ...
    setDesktopView(nextView === "assign" ? "assign" : nextView === "supplier" ? "supplier" : "detail");
  }
  ...
  function renderAssignmentForm(compactForm = false) {
    ...
    return (
      <form className={`dispatch-assignment border border-line bg-white ${compactForm ? "p-4 pb-28" : "p-5"}`} onSubmit={assignOrder}>
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-sm font-extrabold text-slate-500">Điều xe cho lệnh</p>
            <h3 className="text-xl font-extrabold leading-tight text-ink">{selectedOrder.code}</h3>
          </div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-2 text-center text-xs font-extrabold text-slate-500">
          {["Chọn xe", "Xác nhận", "Hoàn tất"].map((label, index) => {
            const active = index <= assignmentStep;
            return (
        ...
        <div className={`${assignmentStep !== 0 ? "hidden" : ""} mt-5 grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1`}>
          {[
        ...
        <div className="mt-5 grid gap-4">
          ...
          {assignmentMode === "company" ? (
            <>
              <section className={assignmentStep !== 0 ? "hidden" : ""}>
                <div className="mb-3 flex items-center justify-between gap-3">
                  <h4 className="font-extrabold text-ink">Chọn xe phù hợp gần nhất</h4>
                  <span className="rounded-full bg-teal-50 px-2 py-0.5 text-xs font-bold text-brand">Đề xuất thông minh</span>
                </div>
                <div className="grid gap-2">
                  {vehicles.map((item, index) => {
                    ...
                    const dayTrips = todayOrders.filter((order) => order.vehicleId === item.id).length;
                    return (
                      <button
                        className={`flex items-start justify-between gap-3 rounded-2xl border p-3.5 text-left transition ${selected ? "border-2 border-brand bg-[#f4fff9] shadow-sm" : "border-line bg-white hover:border-teal-200"} ${item.status !== "active" ? "opacity-60" : ""}`}
                      ...
                      >
                        <span className="flex min-w-0 items-center gap-3">
                          <span className={`grid size-10 shrink-0 place-items-center rounded-xl ${selected ? "bg-teal-50 text-brand" : "bg-slate-100 text-slate-600"}`}><Car size={21} /></span>
                          <span className="min-w-0">
                            <span className="block truncate font-extrabold text-ink">{item.plateNo} {index === 0 && <span className="ml-1 rounded bg-brand px-1.5 py-0.5 text-[10px] text-white">Tối ưu nhất</span>}</span>
                            <span className="block truncate text-xs font-semibold text-slate-500">{item.type} · {item.seats} chỗ</span>
                            <span className="block truncate text-[11px] text-slate-500">Tài xế: <strong className="text-slate-800">{defaultDriver?.fullName ?? "Chưa gán mặc định"}</strong></span>
                            <span className={`block truncate text-[11px] font-medium ${dayTrips === 0 ? "text-brand" : "text-amber-700"}`}>{dayTrips === 0 ? "Đang rảnh trong ngày" : `${dayTrips} chuyến hôm nay`}</span>
                          </span>
                        </span>
                      ...
                      >
                        <span className="flex min-w-0 items-center gap-3">
                          ...
                          <span className="min-w-0">
                            ...
                            <span className="block truncate text-xs font-semibold text-slate-500">{item.phone} · {todayOrders.filter((order) => order.driverId === item.id).length} chuyến hôm nay</span>
                          </span>
                        </span>
                        <span className="flex shrink-0 items-center gap-2">
              ...
              <section className={`${assignmentStep !== 1 ? "hidden" : ""} rounded-2xl border border-line bg-slate-50 p-4`}>
              ...
              <div className={assignmentStep !== 1 ? "hidden" : ""}><Field label="Ghi chú phân công"><textarea className={textAreaClass()} name="reason" placeholder="Ghi chú phân công" /></Field></div>
            </>
          ) : (
            <div className="grid gap-3 md:grid-cols-2">
        ...
        {assignmentStep === 0 && assignmentMode === "company" ? <button className="dispatch-primary mt-5" disabled={!canSubmitAssignment} onClick={() => setAssignmentStep(1)} type="button">Tiếp tục <ChevronRight size={17} /></button> : <button className="dispatch-primary mt-5" disabled={!canSubmitAssignment || isActionPending(`dispatch:assign:${selectedOrder.id}`)} type="submit">
          <Car size={19} /> {isActionPending(`dispatch:assign:${selectedOrder.id}`) ? "Đang điều xe..." : "Xác nhận điều xe"}
        </button>}
        {assignmentStep === 1 && <button className="mt-3 w-full py-2 text-sm text-brand" onClick={() => setAssignmentStep(0)} type="button">Quay lại chọn xe</button>}

        {false && currentAssignmentHistory.length > 0 && (
  ...
  function statusPillClass(order: DispatchOrder) {
    if (order.dispatchStatus === "waiting_assignment") return "border-amber-200 bg-amber-50 text-amber-700";
    if (order.dispatchStatus === "in_progress") return "border-emerald-200 bg-emerald-50 text-emerald-700";
    if (order.dispatchStatus === "completed") return "border-slate-200 bg-slate-100 text-slate-600";
    if (order.dispatchStatus === "cancelled") return "border-rose-200 bg-rose-50 text-rose-700";
    return "border-blue-200 bg-blue-50 text-blue-700";
  }

  function renderSupplierEditForm(compactForm = false) {
    const transport = {
      driverName: selectedOrder.externalDriverName || selectedOrder.driverFullName || driver?.fullName || "",
      driverPhone: selectedOrder.externalDriverPhone || selectedOrder.driverPhone || driver?.phone || "",
      ...
      vehiclePlate: selectedOrder.externalVehiclePlate || selectedOrder.vehiclePlateNo || vehicle?.plateNo || ""
    };
      ...
      <form className={`dispatch-supplier-form rounded-lg border border-line bg-white shadow-sm ${compactForm ? "pb-24" : ""}`} onSubmit={updateOrder}>
        ...
        <div className="grid gap-3 p-4 sm:grid-cols-2">
          <Field label="Hình thức xe">
            <select className={inputClass()} defaultValue={editVehicleOwnership} name="vehicleOwnership">
              <option value="company">Công ty</option>
              <option value="rented">Thuê ngoài/Hợp tác</option>
            </select>
          </Field>
          ...
          <Field label="Xuất HĐ đầu vào">
            <select className={inputClass()} defaultValue={editSupplierInvoiceRequired ? "yes" : "no"} name="supplierInvoiceRequired">
              <option value="yes">Có</option>
              <option value="no">Không</option>
            </select>
          </Field>
          <Field label="Tổng tiền mua gồm VAT"><input className={inputClass()} defaultValue={selectedOrder.supplierTotalWithVat ?? selectedOrder.vehicleCost ?? 0} min="0" name="supplierTotalWithVat" type="number" /></Field>
          <Field label="Đơn vị sở hữu/NCC"><input className={inputClass()} defaultValue={selectedOrder.supplierCompanyName ?? transport.supplierCompanyName ?? ""} name="supplierCompanyName" /></Field>
          <Field label="MST NCC"><input className={inputClass()} defaultValue={selectedOrder.supplierTaxCode ?? transport.supplierTaxCode ?? ""} name="supplierTaxCode" /></Field>
          <Field label="SĐT NCC"><input className={inputClass()} defaultValue={selectedOrder.supplierPhone ?? transport.supplierPhone ?? ""} name="supplierPhone" /></Field>
          <Field label="TK NCC"><input className={inputClass()} defaultValue={selectedOrder.supplierBankAccount ?? transport.supplierBankAccount ?? ""} name="supplierBankAccount" /></Field>
          <Field label="Ngân hàng NCC"><input className={inputClass()} defaultValue={selectedOrder.supplierBankName ?? transport.supplierBankName ?? ""} name="supplierBankName" /></Field>
          <div className="sm:col-span-2">
            <Field label="Địa chỉ NCC"><input className={inputClass()} defaultValue={selectedOrder.supplierAddress ?? transport.supplierAddress ?? ""} name="supplierAddress" /></Field>
          </div>
          <div className="sm:col-span-2">
            <Field label="Ghi chú điều hành"><textarea className={`${inputClass()} min-h-20 resize-none py-2`} defaultValue={selectedOrder.salesNote ?? ""} name="salesNote" placeholder="Ví dụ: bổ sung hồ sơ NCC sau khi xe thuê ngoài đã chạy..." /></Field>
          </div>
          <input name="editReason" type="hidden" value="Dispatcher updated supplier profile" />
        </div>
      ...
      <section className="rounded-xl border border-line bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 overflow-x-auto">
          <div className="min-w-[760px] overflow-hidden rounded-lg border border-line">
            ...
            {rows.filter((row) => {
              const item = vehicles.find((vehicle) => vehicle.id === row.id);
              return (!seatFilter || String(item?.seats) === seatFilter) && (!vehicleStatusFilter || (vehicleStatusFilter === "busy" ? row.orders.length > 0 : vehicleStatusFilter === "free" ? row.orders.length === 0 && item?.status === "active" : item?.status !== "active"));
            }).map((row) => (
              <div className="grid grid-cols-[120px_1fr] border-t border-line" key={row.id}>
                ...
                <div className="relative min-h-16 bg-white" style={{ height: Math.max(64, row.orders.length * 46 + 12) }}>
                  ...
                  {row.orders.map((order, index) => {
                    ...
                    const width = `${Math.max(0.5, ((end - start) / windowSize) * 100)}%`;
                    ...
                    return (
                      <button
                        ...
                        style={{ left, width, top: 6 + index * 46 }}
                        type="button"
                      >
                        <span className="block truncate">{routeSummaryForOrder(order)}</span><span className="block font-normal">{timeOnly(order.startAt)} - {timeOnly(order.endAt)}</span>
                      </button>
                    );
                  })}
                </div>
    ...
    return (
      <div className="space-y-3 pb-24">
        ...
        {scheduleMode === "vehicle" ? (
          ...
          <section className="space-y-2">
            {timelineOrders.map((order) => (
              <button className="grid w-full grid-cols-[54px_minmax(0,1fr)] gap-3 rounded-lg border border-slate-200 bg-white p-3 text-left shadow-sm" key={order.id} onClick={() => selectOrder(order.id, "detail")} type="button">
                ...
                <span className="min-w-0 overflow-hidden">
                  <span className="block max-w-full truncate text-[13px] font-extrabold leading-5 text-slate-900">{routeSummaryForOrder(order)}</span>
                  <span className="mt-1 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 text-[11px] font-semibold text-slate-500">
                    <span className="min-w-0 truncate">{order.code}</span>
                    <span className={`inline-flex h-[22px] shrink-0 items-center rounded-md border px-2 text-[10px] font-extrabold leading-none whitespace-nowrap ${statusPillClass(order)}`}>{dispatchLabels[order.dispatchStatus]}</span>
                  </span>
                </span>
              </button>
            ))}
        ...
        <button
          ...
          type="button"
        >
          <Plus className="mr-2 inline" size={18} />Gán vào lệnh mới
        </button>
      </div>
    );
  }
  ...
  function renderMobileVehicleDetail() {
    const timelineOrders = [...selectedScheduleOrders].sort((a, b) => new Date(a.startAt).getTime() - new Date(b.startAt).getTime());
    const timelineBlocks: Array<{ kind: "gap" | "trip"; start: number; end: number; order?: DispatchOrder }> = [];
    let cursor = 6 * 60;
    timelineOrders.forEach((order) => {
      const start = Math.max(6 * 60, hourOffset(order.startAt));
      const end = Math.min(24 * 60, hourOffset(order.endAt));
      if (start > cursor) timelineBlocks.push({ kind: "gap", start: cursor, end: start });
      timelineBlocks.push({ kind: "trip", start, end, order });
      cursor = Math.max(cursor, end);
    });
    if (cursor < 24 * 60) timelineBlocks.push({ kind: "gap", start: cursor, end: 24 * 60 });
    if (timelineBlocks.length === 0) timelineBlocks.push({ kind: "gap", start: 6 * 60, end: 24 * 60 });
    const workedHours = Math.round(timelineOrders.reduce((sum, order) => sum + durationMinutes(order), 0) / 60);
    ...
    return (
      <section className="space-y-3 pb-24">
        ...
        <label className="dispatch-date flex items-center justify-between">
          <span className="flex items-center gap-2 font-semibold text-slate-800"><CalendarClock size={16} className="text-brand" />{dateOnly(calendarDay.toISOString())}</span>
          <input aria-label="Ngày lịch xe" className="absolute opacity-0" type="date" value={inputDateValue(calendarDay)} onChange={(event) => setCalendarDay(new Date(`${event.target.value}T00:00:00`))} />
          <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold text-emerald-800">Đã chạy: {workedHours}h / 24h</span>
        </label>
        <div className="relative rounded-xl bg-white px-3 py-3 shadow-sm">
          <div className="absolute left-[61px] top-6 bottom-6 w-px bg-slate-200" />
          <div className="space-y-4">
            {timelineBlocks.map((block, index) => {
              const isTrip = block.kind === "trip" && block.order;
              const tone = isTrip && block.order?.dispatchStatus === "in_progress" ? "emerald" : isTrip ? "blue" : "slate";
              return (
                <article className="relative z-10 grid grid-cols-[42px_12px_1fr] gap-3" key={`${block.kind}-${block.start}-${index}`}>
                  <div className="pt-1 text-right">
                    <span className={`text-xs font-bold ${tone === "emerald" ? "text-emerald-700" : tone === "blue" ? "text-blue-700" : "text-slate-500"}`}>{minuteLabel(block.start)}</span>
                    <span className="block text-[10px] text-slate-400">{minuteLabel(block.end)}</span>
                  </div>
                  <div className="pt-2">
                    <span className={`block size-3 rounded-full border-2 ${isTrip ? tone === "emerald" ? "border-emerald-500 bg-emerald-500 ring-2 ring-emerald-100" : "border-blue-600 bg-blue-600 ring-2 ring-blue-100" : "border-slate-300 bg-white"}`} />
                  </div>
                  {isTrip ? (
                    <button className={`rounded-xl border p-3 text-left shadow-sm ${tone === "emerald" ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-blue-200 bg-blue-50 text-blue-900"}`} onClick={() => selectOrder(block.order!.id, "detail")} type="button">
                      <div className="flex items-center justify-between gap-2">
                        <span className={`rounded-md px-2 py-0.5 text-[10.5px] font-bold uppercase text-white ${tone === "emerald" ? "bg-emerald-600" : "bg-blue-600"}`}>{block.order!.dispatchStatus === "in_progress" ? "Đang vận hành" : dispatchLabels[block.order!.dispatchStatus]}</span>
                        <span className="text-[11px] font-bold">{timeOnly(block.order!.startAt)} - {timeOnly(block.order!.endAt)}</span>
                      </div>
                      <p className="mt-2 truncate text-[15px] font-bold">{routeSummaryForOrder(block.order!)}</p>
                      <p className="mt-1 truncate text-xs font-semibold">#{block.order!.code} · {block.order!.guestCount ?? "-"} khách</p>
                    </button>
                  ) : (
                    <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-3">
                      <div><p className="text-[13px] font-semibold text-slate-700">Trống lịch · {Math.max(0, Math.round((block.end - block.start) / 60))}h</p><p className="text-[11.5px] text-slate-500">Sẵn sàng nhận lệnh</p></div>
                      <button className="rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[11.5px] font-semibold text-emerald-700" onClick={() => { setAssignmentStep(0); setMobileView("assign"); }} type="button">Gán lệnh</button>
                    </div>
                  )}
                </article>
              );
        ...
        <button className="fixed inset-x-4 bottom-4 z-20 h-12 rounded-xl bg-brand text-sm font-bold text-white shadow-lg shadow-emerald-700/20" onClick={() => { setAssignmentStep(0); setMobileView("assign"); }} type="button"><Plus className="mr-2 inline" size={18} />Gán vào lệnh mới</button>
      </section>
    );
  }
    ...
    return (
      <header className="flex min-h-[72px] flex-wrap items-center justify-between gap-3 border-b border-line bg-white px-6 py-3">
        ...
        <div className="flex min-w-0 flex-[2] flex-wrap items-center justify-end gap-2">
          ...
          <button className="relative grid size-10 place-items-center rounded-full border border-line bg-white text-ink" onClick={() => setShowDispatchAlerts(!showDispatchAlerts)} type="button">
            <Bell size={18} />
      ...
      <section className="dispatch-detail-card rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-3 rounded-xl bg-slate-100 p-1">
          {[
            ...
            <button className={`h-8 rounded-lg px-2 text-xs font-bold ${detailTab === key ? "bg-brand text-white" : "text-slate-500"}`} key={key} onClick={() => setDetailTab(key as typeof detailTab)} type="button">{label}</button>
          ))}
        </div>
        {detailTab === "overview" && (
          <div className="mt-4 space-y-3">
            ...
            <div className="rounded-xl border border-line bg-white p-3">
              ...
              <InfoLine label="Xác nhận tài xế" value={driverAckLabel} />
            </div>
          </div>
        )}
        {detailTab === "route" && (
        ...
        {selectedOrder.orderStatus === "pending_dispatch_review" && (
          <div className="mt-4 grid grid-cols-2 gap-2">
            <button className="h-10 rounded-lg bg-brand text-sm font-bold text-white disabled:bg-slate-300" disabled={!canAssignVehicle || isActionPending(`dispatch:review:${selectedOrder.id}:approved`)} onClick={() => reviewDispatchProposal(selectedOrder.id, "approved", "")} type="button">Duyệt</button>
            <button className="h-10 rounded-lg border border-rose-200 bg-rose-50 text-sm font-bold text-rose-700 disabled:bg-slate-100" disabled={!canAssignVehicle || isActionPending(`dispatch:review:${selectedOrder.id}:rejected`)} onClick={() => reviewDispatchProposal(selectedOrder.id, "rejected", "Điều hành từ chối")} type="button">Từ chối</button>
          </div>
        )}
        <button
          className="mt-4 inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg border border-teal-200 bg-teal-50 px-4 text-sm font-extrabold text-brand hover:bg-teal-100"
        ...
        <button
          ...
          onClick={() => { setAssignmentStep(0); setDesktopView("assign"); setMobileView("assign"); }}
          type="button"
        >
        ...
        <section className="space-y-4 rounded-2xl border border-line bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2 overflow-x-auto">
              {["Tất cả", "Chờ điều xe", "Sắp chạy", "Đang chạy"].map((label) => (
                <button className={`h-9 rounded-full px-4 text-sm font-extrabold ${queueFilter === label ? "bg-brand text-white" : "bg-slate-100 text-slate-600"}`} key={label} onClick={() => setQueueFilter(label)} type="button">{label}</button>
              ))}
            </div>
          </div>
          <div className="grid gap-3">
    ...
    return (
      ...
      <section className="rounded-2xl border border-line bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            ...
            <Field label="Loại xe"><select className={inputClass()} value={seatFilter} onChange={(event) => setSeatFilter(event.target.value)}><option value="">Tất cả</option>{Array.from(new Set(vehicles.map((vehicle) => vehicle.seats))).sort((a, b) => a - b).map((seats) => <option key={seats} value={seats}>{seats} chỗ</option>)}</select></Field>
            <Field label="Trạng thái"><select className={inputClass()} value={vehicleStatusFilter} onChange={(event) => setVehicleStatusFilter(event.target.value)}><option value="">Tất cả</option><option value="busy">Có lịch</option><option value="free">Trống lịch</option><option value="inactive">Ngừng hoạt động</option></select></Field>
          </div>
          <button className="mt-4 text-sm font-extrabold text-brand" onClick={() => { setSeatFilter(""); setVehicleStatusFilter(""); }} type="button">Xóa bộ lọc</button>
    ...
    return (
      <header className="dispatch-mobile-header bg-white">
        {mobileView === "overview" ? (
          ...
          <>
            <button className="dispatch-icon-button" aria-label="Quay lại" onClick={() => { if (mobileView === "assign" && assignmentStep > 0) setAssignmentStep(0); else setMobileView(mobileView === "vehicleDetail" ? "schedule" : mobileView === "assign" || mobileView === "supplier" ? "detail" : mobileView === "detail" ? "orders" : "overview"); }} type="button"><ChevronLeft size={22} /></button>
  ...
  function renderMobileOverview() {
    const mobileStats = [
      { label: "Lệnh hôm nay", value: todayOrders.length, detail: `${dispatchBaseQueue.length} lệnh cần theo dõi`, tone: "teal", icon: ClipboardList },
      { label: "Chờ điều xe", value: waitingOrders.length, detail: `${urgentOrders.length} lệnh cần gấp`, tone: "orange", icon: Clock3 },
      { label: "Đang chạy", value: runningOrders.length, detail: "Trên hành trình", tone: "green", icon: Navigation },
    ...
    return (
      <div className="dispatch-overview space-y-4 pb-24">
        ...
        <div className="dispatch-metrics grid grid-cols-2 gap-3">
  ...
  function renderMobileOrders() {
    const filterCounts: Record<string, number> = {
      "Tất cả": dispatchBaseQueue.length,
      "Chờ điều xe": dispatchBaseQueue.filter((order) => order.dispatchStatus === "waiting_assignment" || order.orderStatus === "pending_dispatch_review").length,
      "Sắp chạy": dispatchBaseQueue.filter((order) => order.dispatchStatus === "assigned" || order.dispatchStatus === "driver_accepted").length,
  ...
  return (
    <section className="dispatch-ui pb-24 lg:pb-0">
      <div className="hidden min-h-screen grid-cols-[170px_minmax(0,1fr)] bg-[#f5f8fa] lg:grid">
            ...
            {desktopView === "schedule"
              ...
              : desktopView === "orders"
                ...
                : desktopView === "supplier"
                  ? (
                    <div className="dispatch-orders-layout">
                      <div>{renderCompactOrderDetail("Đang điều xe")}</div>
                      <div className="sticky top-5">{renderSupplierEditForm()}</div>
                    </div>
                  )
                : desktopView === "assign"
                  ? renderDesktopAssign()
      ...
      <div className="dispatch-mobile-shell lg:hidden">
        ...
        <main className="dispatch-mobile-main">
          ...
          {mobileView === "supplier" && renderSupplierEditForm(true)}
        </main>
...
function FinancePanel({
  ...
  const [financeView, setFinanceView] = useState<"queue" | "profile" | "payment" | "documents" | "preview" | "close">("queue");
  const [financeSearch, setFinanceSearch] = useState("");
  const [financeFilter, setFinanceFilter] = useState<"all" | "debt" | "invoice" | "supplier" | "driver" | "close">("all");
  const paymentTargetAmount = debt || selectedOrder.amountDue;
  const [financePaymentDraft, setFinancePaymentDraft] = useState<{ amount: string; orderId: string } | null>(null);
  const financePaymentAmount = financePaymentDraft?.orderId === selectedOrder.id ? financePaymentDraft.amount : String(paymentTargetAmount);
  const setFinancePaymentAmount = (amount: string) => setFinancePaymentDraft({ amount, orderId: selectedOrder.id });
  const parsedFinancePaymentAmount = Number(financePaymentAmount) || 0;
  const remainingAfterFinancePayment = Math.max(paymentTargetAmount - parsedFinancePaymentAmount, 0);
  const financeCounts = {
    all: financeQueue.length,
    debt: financeQueue.filter((order) => order.paymentStatus !== "paid").length,
    invoice: financeQueue.filter((order) => order.invoiceStatus !== "issued" && order.invoiceStatus !== "not_required").length,
    supplier: financeQueue.filter((order) => order.vehicleOwnership === "rented").length,
    driver: financeQueue.filter((order) => order.driverReportStatus === "reported" && (order.driverCollectedAmount ?? 0) > 0).length,
    close: financeQueue.filter((order) => order.reconciliationStatus !== "closed").length
  };
  const filteredFinanceQueue = financeQueue.filter((order) => {
    const haystack = `${order.code} ${order.customerName} ${routeSummaryForOrder(order)}`.toLowerCase();
    const matchesSearch = haystack.includes(financeSearch.trim().toLowerCase());
    const matchesFilter =
      financeFilter === "all" ||
      (financeFilter === "debt" && order.paymentStatus !== "paid") ||
      (financeFilter === "invoice" && order.invoiceStatus !== "issued" && order.invoiceStatus !== "not_required") ||
      (financeFilter === "supplier" && order.vehicleOwnership === "rented") ||
      (financeFilter === "driver" && order.driverReportStatus === "reported" && (order.driverCollectedAmount ?? 0) > 0) ||
      (financeFilter === "close" && order.reconciliationStatus !== "closed");
    return matchesSearch && matchesFilter;
  });
  const closeChecks = [
    { label: "Chuyến đi đã hoàn thành", ok: selectedOrder.dispatchStatus === "completed" },
    { label: "Dòng tiền đã ghi nhận", ok: selectedOrder.paymentStatus !== "refunded" },
    { label: "Hóa đơn/chứng từ đã xử lý", ok: invoiceReady },
    { label: "Báo cáo tài xế đã đối chiếu", ok: selectedOrder.driverReportStatus !== "reported" || selectedOrder.reconciliationStatus === "closed" },
  ...
  const financeCardClass = "rounded-2xl border border-slate-200/80 bg-white shadow-[0_2px_10px_rgba(15,23,42,0.04)]";
  const financePrimaryButtonClass = "inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#0a9b81] px-4 text-sm font-bold text-white shadow-lg shadow-teal-700/20 hover:bg-[#087e69] disabled:cursor-not-allowed disabled:bg-slate-300";
  const financeSoftButtonClass = "inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-teal-200 bg-teal-50 px-4 text-sm font-semibold text-[#0a7f6a] hover:bg-teal-100";
  const filterChips: Array<{ id: typeof financeFilter; label: string }> = [
    { id: "all", label: "Tất cả" },
    { id: "debt", label: "Còn nợ" },
    { id: "invoice", label: "Chờ hóa đơn" },
    { id: "supplier", label: "NCC" },
    { id: "driver", label: "Thu hộ" },
    { id: "close", label: "Đối soát" }
  ];
  const navItems: Array<{ id: typeof financeView; label: string; icon: typeof ClipboardList }> = [
    { id: "queue", label: "Cần xử lý", icon: ClipboardList },
  ...
  const renderFinanceHeader = (title: string, subtitle?: string) => (
    <header className="sticky top-0 z-20 border-b border-slate-100 bg-white/95 px-4 py-3 backdrop-blur lg:static lg:rounded-2xl lg:border lg:border-slate-200/80">
      <div className="flex items-center justify-between gap-3">
        <button className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-slate-600 hover:bg-slate-100" onClick={() => setFinanceView("queue")} type="button">
          <ChevronLeft size={21} />
        </button>
        <div className="min-w-0 flex-1 text-center lg:text-left">
          <p className="text-[11px] font-semibold text-slate-400">Kế toán • {dateOnly(selectedOrder.startAt)}</p>
        ...
        <button className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-50 text-slate-600" type="button">
          <Bell size={18} />
  ...
  const renderQueueCard = (order: DispatchOrder) => {
    ...
    return (
      <article className={`${financeCardClass} p-4`} key={order.id}>
        <button className="w-full text-left" onClick={() => selectFinanceOrder(order.id)} type="button">
          <div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-3">
            <div className="min-w-0">
              <p className="truncate text-[13px] font-extrabold text-[#0a9b81]">{order.code}</p>
              <p className="mt-1 truncate text-xs font-medium text-slate-400">{timeOnly(order.startAt)} • {dateOnly(order.startAt)}</p>
            </div>
            <ChevronRight className="mt-1 shrink-0 text-slate-300" size={18} />
          </div>
          <p className="mt-3 line-clamp-2 text-sm font-bold leading-snug text-slate-900">{routeSummaryForOrder(order)}</p>
          <p className="mt-1 truncate text-xs font-medium text-slate-500">{order.customerName} • {order.customerKind === "individual" ? "Cá nhân" : "Doanh nghiệp"}</p>
          <div className="mt-3 flex items-center justify-between gap-3 border-t border-dashed border-slate-100 pt-3">
            <p className="text-sm font-extrabold text-slate-950">{money(order.amountDue)}</p>
            <Badge tone={orderDebt > 0 ? "warn" : "good"}>{orderDebt > 0 ? `Còn ${money(orderDebt)}` : "Đã thu"}</Badge>
          </div>
          {issues.length > 0 && (
  ...
  const renderQueue = () => (
    <div className="space-y-4">
      {renderFinanceHeader("Cần xử lý", "Quản lý và theo dõi các hồ sơ cần xử lý")}
      <section className="grid grid-cols-3 gap-2.5">
        <div className={`${financeCardClass} border-rose-100 p-3 text-center`}><p className="text-2xl font-black text-rose-500">{financeCounts.debt}</p><p className="text-[11px] font-bold text-slate-600">Còn nợ</p></div>
        <div className={`${financeCardClass} border-amber-100 p-3 text-center`}><p className="text-2xl font-black text-amber-500">{financeCounts.invoice}</p><p className="text-[11px] font-bold text-slate-600">Chờ hóa đơn</p></div>
        <div className={`${financeCardClass} border-teal-100 p-3 text-center`}><p className="text-2xl font-black text-[#0a9b81]">{financeCounts.close}</p><p className="text-[11px] font-bold text-slate-600">Đối soát</p></div>
      </section>
      <section className="space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
            <input className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-9 pr-3 text-sm font-medium outline-none focus:border-[#0a9b81] focus:ring-2 focus:ring-teal-100" onChange={(event) => setFinanceSearch(event.target.value)} placeholder="Tìm mã lệnh, khách hàng, tuyến..." value={financeSearch} />
          </div>
          <button className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600" type="button"><Settings2 size={17} /></button>
        </div>
        <div className="no-scrollbar flex gap-2 overflow-x-auto pb-1">
          {filterChips.map((chip) => (
            <button className={`whitespace-nowrap rounded-full px-3.5 py-1.5 text-xs font-bold ${financeFilter === chip.id ? "bg-[#0a9b81] text-white" : "border border-slate-200 bg-white text-slate-600"}`} key={chip.id} onClick={() => setFinanceFilter(chip.id)} type="button">
              {chip.label} ({financeCounts[chip.id]})
            </button>
          ))}
        </div>
      </section>
      <section className="space-y-3">
        {filteredFinanceQueue.length === 0 ? <p className={`${financeCardClass} p-4 text-sm text-slate-500`}>Không có hồ sơ phù hợp.</p> : filteredFinanceQueue.map(renderQueueCard)}
      </section>
    </div>
  );
  ...
  const renderProfile = () => (
    <div className="space-y-4">
      {renderFinanceHeader("Hồ sơ lệnh", selectedOrder.code)}
      <section className={`${financeCardClass} space-y-3.5 p-4`}>
        <div className="flex items-center justify-between gap-3">
          <Badge tone="info">{selectedOrder.code}</Badge>
          <Badge tone={debt > 0 ? "warn" : "good"}>{paymentLabels[selectedOrder.paymentStatus]}</Badge>
        </div>
        <p className="line-clamp-2 text-xl font-extrabold leading-tight text-slate-950">{routeSummaryForOrder(selectedOrder)}</p>
        <p className="text-xs font-semibold text-slate-500">{formatDateTime(selectedOrder.startAt)} - {formatDateTime(selectedOrder.endAt)}</p>
        <div className="grid grid-cols-3 divide-x divide-slate-100 rounded-2xl border border-slate-100 bg-slate-50 p-3 text-center">
      ...
      <section className={`${financeCardClass} p-4`}>
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">Lộ trình hoàn tất hồ sơ</h3>
          <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-600">{closeCheckCount}/{closeChecks.length} bước</span>
        </div>
        <div className="relative mt-3 space-y-3">
          {[
            { label: "Thu tiền", hint: debt > 0 ? "Cần ghi nhận thanh toán" : "Đã đủ tiền", view: "payment" as const, ok: debt <= 0 },
            { label: "Chứng từ", hint: "Kiểm tra hóa đơn, NCC, thu hộ", view: "documents" as const, ok: invoiceReady },
            { label: "Preview final", hint: "Rà soát trước khi gửi lệnh", view: "preview" as const, ok: true },
            { label: "Đóng hồ sơ", hint: "Hoàn tất tài chính", view: "close" as const, ok: canCloseSelectedOrder }
          ].map((step, index) => (
            <button className="flex w-full items-center justify-between gap-3 rounded-xl p-2 text-left hover:bg-slate-50" key={step.label} onClick={() => setFinanceView(step.view)} type="button">
              <div className="flex items-center gap-3">
                <span className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-black ${step.ok ? "bg-[#0a9b81] text-white" : "border border-slate-200 bg-slate-100 text-slate-500"}`}>{index + 1}</span>
                <span><span className="block text-sm font-bold text-slate-900">{step.label}</span><span className="block text-xs text-slate-500">{step.hint}</span></span>
              </div>
      ...
      <button className={financePrimaryButtonClass} onClick={() => setFinanceView(debt > 0 ? "payment" : "documents")} type="button">{debt > 0 ? "Ghi nhận thanh toán" : "Kiểm tra chứng từ"}</button>
    </div>
  );

  const renderPayment = () => (
    <form className="space-y-4" onSubmit={recordPayment}>
      {renderFinanceHeader("Ghi nhận thanh toán", selectedOrder.code)}
      <section className={`${financeCardClass} grid grid-cols-2 divide-x divide-slate-100 p-4`}>
        <div className="pr-3"><p className="text-xs font-bold uppercase text-slate-400">Khách hàng</p><p className="mt-1 truncate text-sm font-bold text-slate-900">{selectedOrder.customerName}</p></div>
        <div className="pl-3"><p className="text-xs font-bold uppercase text-slate-400">Mã lệnh</p><p className="mt-1 truncate text-sm font-bold text-slate-900">{selectedOrder.code}</p></div>
      </section>
      <section className={`${financeCardClass} space-y-3 p-4`}>
        <div className="flex items-center justify-between">
          <label className="text-xs font-bold uppercase text-slate-500" htmlFor="finance-payment-amount">Số tiền thu</label>
          <span className="text-xs font-bold text-[#0a9b81]">Phải thu: {money(paymentTargetAmount)}</span>
        </div>
        <div className="flex items-center rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 focus-within:border-[#0a9b81] focus-within:ring-2 focus-within:ring-teal-100">
          <input className="w-full border-0 bg-transparent p-0 text-2xl font-black text-slate-950 outline-none focus:ring-0" id="finance-payment-amount" min="1" name="amount" onChange={(event) => setFinancePaymentAmount(event.target.value)} required type="number" value={financePaymentAmount} />
          <span className="ml-2 text-lg font-bold text-slate-400">đ</span>
        </div>
        <div className="grid grid-cols-3 gap-2">
          <button className="rounded-xl bg-[#0a9b81] px-3 py-2 text-xs font-bold text-white" onClick={() => setFinancePaymentAmount(String(paymentTargetAmount))} type="button">Thu đủ</button>
          <button className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700" onClick={() => setFinancePaymentAmount(String(Math.round(paymentTargetAmount / 2)))} type="button">50%</button>
          <button className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700" type="button">Tùy chỉnh</button>
        </div>
      </section>
      <section className={`${financeCardClass} space-y-3 p-4`}>
        <Field label="Phương thức"><select className={inputClass()} name="method"><option value="bank_transfer">Chuyển khoản</option><option value="cash">Tiền mặt</option><option value="card">Thẻ</option><option value="other">Khác</option></select></Field>
        <Field label="Ngày giờ thu"><input className={inputClass()} defaultValue={vietnamDateTimeLocalValue()} name="paidAt" type="datetime-local" /></Field>
        <Field label="Tài khoản nhận"><input className={inputClass()} defaultValue={selectedOrder.collectionBankAccount ?? ""} name="bankAccount" placeholder="STK nhận tiền" /></Field>
        <Field label="Ngân hàng"><input className={inputClass()} defaultValue={selectedOrder.collectionBankName ?? ""} name="bankName" placeholder="Ngân hàng nhận" /></Field>
        <Field label="Người/đơn vị thu"><input className={inputClass()} defaultValue={selectedOrder.collectionAccountOwner ?? "Công ty thu"} name="collector" /></Field>
        <Field label="Mã giao dịch"><input className={inputClass()} name="reference" placeholder="Nếu có" /></Field>
        <Field label="Ghi chú"><textarea className={textAreaClass()} name="note" placeholder="Khách chuyển khoản, tài xế thu hộ..." /></Field>
      </section>
      <div className={`${financeCardClass} flex items-center justify-between bg-emerald-50/70 p-4`}>
        <span className="text-sm font-bold text-slate-700">Còn lại sau thu</span>
        <span className="text-base font-extrabold text-[#0a9b81]">{money(remainingAfterFinancePayment)}</span>
      </div>
      <button className={financePrimaryButtonClass} disabled={!canRecordPayment || isActionPending(`finance:payment:${selectedOrder.id}`)} type="submit">
        <CheckCircle2 size={18} /> {isActionPending(`finance:payment:${selectedOrder.id}`) ? "Đang ghi..." : `Xác nhận đã thu ${money(parsedFinancePaymentAmount)}`}
      </button>
    </form>
  );
  ...
  const renderDocuments = () => (
    <div className="space-y-4">
      {renderFinanceHeader("Chứng từ", selectedOrder.code)}
      <section className={`${financeCardClass} border-teal-100 bg-teal-50/60 p-4`}>
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-100 text-[#0a9b81]"><CheckCircle2 size={20} /></span>
          <div>
            <p className="text-sm font-bold text-teal-950">Kiểm tra chứng từ theo dữ liệu hệ thống</p>
            <p className="text-xs font-medium text-teal-700">Không dùng upload ảnh chứng từ ở màn kế toán.</p>
          </div>
        </div>
      </section>
      {[
        {
          ...
          icon: ReceiptText,
          tone: invoiceReady ? "good" : "warn",
          status: invoiceLabels[selectedOrder.invoiceStatus],
          rows: [
            ["Tên khách/công ty", selectedOrder.companyName || selectedOrder.customerName],
            ["MST", selectedOrder.taxCode || "Không yêu cầu"],
            ["Email nhận HĐ", selectedOrder.billingEmail || "-"]
          ]
        },
        {
          title: "NCC / xe thuê ngoài",
          icon: Car,
          tone: selectedOrder.vehicleOwnership === "rented" ? "info" : "neutral",
          status: selectedOrder.vehicleOwnership === "rented" ? "Có áp dụng" : "Xe công ty",
          rows: [
            ["Nhà cung cấp", selectedOrder.supplierCompanyName || selectedOrder.supplierOwnerName || "-"],
            ["Tổng tiền mua", money(selectedSupplierPayable)],
            ["Hóa đơn đầu vào", selectedOrder.supplierInvoiceRequired ? "Có yêu cầu" : "Không yêu cầu"],
            ["MST NCC", selectedOrder.supplierTaxCode || "-"]
          ]
        },
        {
          title: "Thu hộ tài xế",
          icon: UserRound,
          tone: selectedDriverHeldAmount > 0 ? "info" : "good",
          status: selectedDriverHeldAmount > 0 ? "Có áp dụng" : "Không phát sinh",
          rows: [
            ["Tài xế thu hộ", money(selectedDriverHeldAmount)],
            ["Trạng thái báo cáo", selectedOrder.driverReportStatus],
            ["Ghi chú", selectedDriverReportNoteParts.collectionNote || "-"]
          ]
        }
      ].map((card) => {
        const Icon = card.icon;
        return (
          <article className={`${financeCardClass} overflow-hidden`} key={card.title}>
            <div className="flex items-start justify-between gap-3 border-b border-slate-100 p-4">
              <div className="flex min-w-0 items-center gap-3">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-50 text-[#0a9b81]"><Icon size={18} /></span>
                <h3 className="truncate text-base font-extrabold text-slate-900">{card.title}</h3>
              </div>
              <Badge tone={card.tone as "neutral" | "good" | "warn" | "danger" | "info"}>{card.status}</Badge>
            </div>
            <div className="space-y-2.5 p-4 text-xs">
              {card.rows.map(([label, value]) => (
                <div className="flex items-start justify-between gap-3" key={label}>
                  <span className="shrink-0 font-semibold text-slate-500">{label}</span>
                  <span className="min-w-0 text-right font-bold text-slate-900 [overflow-wrap:anywhere]">{value}</span>
                </div>
              ))}
            </div>
          </article>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        <button className={financeSoftButtonClass} disabled={!canUpdateInvoice || isActionPending(`finance:invoice:${selectedOrder.id}:ready_to_issue`)} onClick={() => updateInvoiceStatus("ready_to_issue")} type="button">Sẵn sàng HĐ</button>
        <button className={financePrimaryButtonClass} disabled={!canUpdateInvoice || isActionPending(`finance:invoice:${selectedOrder.id}:issued`)} onClick={() => updateInvoiceStatus("issued")} type="button">Đã xuất HĐ</button>
      </div>
    </div>
  );
  ...
  const renderClose = () => (
    <div className="space-y-4">
      {renderFinanceHeader("Đối soát & đóng hồ sơ", selectedOrder.code)}
      <section className={`${financeCardClass} border-emerald-200 bg-emerald-50/70 p-4`}>
        <div className="flex items-center gap-3">
          <span className="flex h-12 w-12 items-center justify-center rounded-full bg-[#0a9b81] text-white shadow-md"><CheckCircle2 size={24} /></span>
          <div className="min-w-0">
            <h3 className="truncate text-base font-extrabold text-emerald-950">{canCloseSelectedOrder ? "Đủ điều kiện đóng hồ sơ" : "Còn điều kiện cần xử lý"}</h3>
            <p className="text-xs font-medium text-emerald-800">{closeCheckCount}/{closeChecks.length} điều kiện đã đạt.</p>
          </div>
        </div>
      </section>
      <section className={`${financeCardClass} p-4`}>
        <h3 className="text-sm font-extrabold text-slate-900">Tổng hợp tài chính thực tế</h3>
        <div className="mt-3 space-y-2 text-sm">
          <div className="flex justify-between gap-3"><span className="text-slate-500">Doanh thu</span><span className="font-bold text-slate-900">{money(selectedOrder.amountDue)}</span></div>
          <div className="flex justify-between gap-3"><span className="text-slate-500">Chi phí</span><span className="font-bold text-slate-900">{money(orderCost(selectedOrder))}</span></div>
          <div className="flex justify-between gap-3 border-t border-dashed border-slate-200 pt-2"><span className="font-bold text-slate-900">Lợi nhuận thực tế</span><span className="font-extrabold text-[#0a9b81]">{money(orderActualProfit(selectedOrder))}</span></div>
        </div>
      </section>
      <section className={`${financeCardClass} p-4`}>
        <h3 className="text-sm font-extrabold text-slate-900">Điều kiện đóng hồ sơ</h3>
        <div className="mt-3 space-y-2.5">
          {closeChecks.map((item) => (
            <p className="flex items-center gap-2 text-sm font-semibold text-slate-700" key={item.label}>
              <span className={`flex h-5 w-5 items-center justify-center rounded-full ${item.ok ? "bg-[#0a9b81] text-white" : "bg-slate-100 text-slate-300"}`}><CheckCircle2 size={14} /></span>
              {item.label}
            </p>
          ))}
        </div>
        {closeBlockers.length > 0 && <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-900">Chưa thể đóng: {closeBlockers.join(", ")}.</p>}
      </section>
      <button className={financePrimaryButtonClass} disabled={!canCloseSelectedOrder || isActionPending(`finance:close:${selectedOrder.id}`)} onClick={reconcileOrder} type="button">
        {isActionPending(`finance:close:${selectedOrder.id}`) ? "Đang đóng..." : "Đóng hồ sơ tài chính"}
      </button>
    </div>
  );

  const renderActiveFinanceView = () => {
    if (financeView === "profile") return renderProfile();
    if (financeView === "payment") return renderPayment();
    if (financeView === "documents") return renderDocuments();
    if (financeView === "preview") return renderPreview();
    if (financeView === "close") return renderClose();
    return renderQueue();
  };
